...
private:
	...
	static void moveBuffer(gpsMessage gpsData); //Сдвинуть буфер

	static double getDistance(gpsMessage *firstCoor,	//Получить расстояние между точками
		gpsMessage *SecondCoor, bool altitud);
public:
	/*Обработать пакет от гпс модуля*/
	static void processPackage(gpsMessage gpsData) ;
